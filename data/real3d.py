...
# voxel_size_setting = 0.2
class Real3D(Dataset):

    def __init__(self, split, class_name):
        self.class_name = class_name
        self.data_path = os.path.join(DATASETS_PATH, self.class_name, split)
...
class Real3DTrain(Real3D):
    ...
    def load_dataset(self):
        ...
        if self.class_name == 'ALL':
            self.class_name = real3d_classes()
        else:
...
class Real3DTest(Real3D):
    ...
    def __getitem__(self, idx):
        ...
        pcd1.points = o3d.utility.Vector3dVector(input_points[idx1,0:3]) #点云数据
        pcd2.points = o3d.utility.Vector3dVector(input_points[idx2,0:3]) #点云数据
